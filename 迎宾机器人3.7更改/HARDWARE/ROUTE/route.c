#include <stdio.h>
//本文档来计算线路  写了个大体思路，配合我的路口检测 
//线路图参数  布线规则：可以直接连接的百位加一，一阶三阶之间不能有连接，同阶不能有连接
//点与点之间的连接是已知的，每个点若是多余一个连接，则有一个字典（可用数组表示也可定义类），由于到了每个点，机器人与点的方向固定，故可知不同分支在左中右的哪一测
//由此判断路口并且进行选择 ，路口选择函数在PID里
